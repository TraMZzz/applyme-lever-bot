...
**Submit run measured (leverdemo sandbox, `--submit-mode sandbox`, 2026-06-11):** a real POST to Lever's `leverdemo` returned **`CAPTCHA_BLOCKED` (`hcaptcha_unverified`)** — clicking submit rendered an interactive hCaptcha challenge (captured in `final.png`); the session stayed on `/apply` (no `/thanks`). This was run **headless in Docker**, and *headless is itself a strong hCaptcha detection signal* — so the silent-pass is not expected to hold there. It confirms two design premises at once: (1) the **silent-pass needs a headful, human-looking session on a clean residential IP** (the dry-run + this submit prove the pipeline; the *pass rate* is a headful measurement), and (2) when a challenge does render, the third-party solvers cannot clear Lever's invisible Enterprise hCaptcha (§4 above), so the honest terminal state is `captcha_blocked`. (Fixing a real submit-path bug en route — it polled the post-submit token, which a silent pass navigates away from — is itself a "what didn't work" item; now navigation-based and bounded.)

**Still to confirm — the load-bearing KPI:** the **in-browser silent-pass *rate*** on a **headful** run (real Chrome, visible window, clean residential IP) — only there does the invisible hCaptcha have a realistic chance of self-solving. Also unmeasured: whether the fabricated data mismatch (resume = pipeline engineer vs profile = Product Manager) triggers any field-level rejection on a real submit. The headful submit run happens on the operator's machine (`--submit-mode sandbox` against `leverdemo` first, then `real` opt-in).
