...
A paid solver (**CapSolver** primary, **2Captcha** fallback, behind one `solve({sitekey, pageurl, isInvisible, rqdata?, proxy?})` interface) is called **only when an interactive challenge actually fires**, and it **fails closed** — with no `rqdata`/proxy captured it raises `SolverUnavailable` rather than firing a doomed request. It is a fallback, not the foundation, for two honest reasons now **verified** (§4): `secure-api.js` is hCaptcha **Enterprise**, where the token *is* a passive risk score graded on the live session — so an out-of-band token is rejected even when `siteverify` passes; and CapSolver **delisted hCaptcha entirely** in 2026 (the rest of the market is degraded after the 2024 legal enforcement). So the design leans on avoiding the challenge and treats solving as insurance — recording `captcha_blocked` honestly when it can't be solved.
...
**Verdict: reliable *unattended* solving of this captcha is not achievable in 2026.** It is feasible *opportunistically* (silent-pass when the session's risk score is clean) but not *deterministically*. A residential proxy is necessary-but-not-sufficient for any solver leg (solve-IP must equal submit-IP) and fixes none of the three failures on its own. If a solver tier is ever revived it must be the in-session "Option C" shape — capture `rqdata` live from `checksiteconfig`, route both the solve and the submit through the *same* residential proxy, use a vendor that still serves hCaptcha, and inject via Lever's own `onSuccess(token)` — for an unverified ≤60% ceiling. The honest design is silent-pass-first with a human hand-off at the challenge (§5).
