...
| `browser/motion` | Recorded-human motion replay: affine-retargets real captured mouse/scroll/keystroke traces; synthetic-Bézier fallback | `browser/human` |
| `app_pw` | Single-vacancy flow: warm Cloudflare → `/apply` → parse → `resolve_answers` → `pw_fill` → dry-run evidence OR submit+captcha+confirmation | pw_engine, pw_fill, motion |
...
- **`motion.py`** *(the behavioural silent-pass lever — REPORT §4a)* — replays **real recorded-human** mouse/scroll/keystroke traces (captured by `scripts/record_motion.py`) instead of synthetic Béziers. `MotionEngine.path_to` picks a recorded gesture and **affine-retargets it** (rotate by the target-vs-recorded angle, scale by the length ratio) so its net displacement maps the required start→end while preserving the recorded velocity profile, overshoot, and inter-sample timing — the exact features hCaptcha's `motionData` grades. **Falls back to `human.bezier_path` when no traces are loaded** (the apply flow never regresses; recorded motion is an opt-in upgrade via `JOOBLE_MOTION_TRACES`). Consumed by `app_pw` (warming/pre-`execute()` dwell) and `lever/pw_fill` (field cursor moves + keystroke timing); `scripts/check_motion.py` smoke-proves it drives real Chrome with human-shaped events. We do **not** ship fabricated "human" traces — authenticity comes only from a real recording.
...
- **`submit.py`** — pure **`classify_outcome(final_url, http_status, body)`**: a success redirect *off* the form (`…/thanks`, **or** `lever.co …?LeverAppId=<uuid>` for tenants like `leverdemo` that have no thanks page — Lever mints the id only on acceptance) → `SUCCESS`; otherwise parse the re-rendered body with `selectolax` → flagged required fields → `FAILED:MISSING_REQUIRED_FIELD:<f>`; else a populated `p.error-message` banner (**excluding the always-present hidden résumé-oversize banner by class *and* its text** — that false-positive once masked the submit-button bug) + no flagged field → `CAPTCHA_BLOCKED:hcaptcha_unverified`; `http_status >= 500` → `RETRYABLE_ERROR`; an unattributable re-render → `FAILED:no_thanks_redirect`. The **submit trigger lives in `app_pw._submit_with_captcha`** (silent-pass first): dismiss the cookie-consent bar, then click Lever's **real visible** submit button (`button.template-btn-submit`, `type=button` — *not* the hidden native `button[type=submit]`, clicking which is a no-op), whose handler runs `hcaptcha.execute()` then POSTs; wait for the post-submit navigation (silent-pass → `/thanks`-or-`LeverAppId`). Only if it doesn't navigate AND a **visible** challenge iframe renders AND a solver key is set → `solve_hcaptcha` → inject the token → re-click. `SUBMIT_MODE` gates whether this terminal step runs at all.
...
Detection: `SUCCESS` = a redirect off the form (`…/thanks`, or `lever.co …?LeverAppId=<uuid>` on `leverdemo`); failure = a 400 re-render (flagged field → `FAILED`, else a *real* error banner → `CAPTCHA_BLOCKED`, else `FAILED:no_thanks_redirect`); `http_status >= 500` → `RETRYABLE_ERROR`. `DUPLICATE_SUSPECTED` is a **defined status literal reserved for the roadmap dedupe ledger** (§11) — the MVP ships no ledger, so it is not emitted yet (Lever merges silently and still returns `/thanks`).
...
├── browser/ pw_engine.py · human.py · motion.py · preflight.py
